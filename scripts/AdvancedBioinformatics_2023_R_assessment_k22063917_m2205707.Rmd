...
```{R question 3.17}
foldEnrichment_first <- firstPeakSet$fold_enrichment
foldEnrichment_second <- secondPeakSet$fold_enrichment
foldEnrichment_firstANDsecondPeakSets <- firstANDsecondPeakSets$fold_enrichment
foldEnrichment_secondANDfirstPeakSets <- secondANDfirstPeakSets$fold_enrichment
```

```{R question 3.17}
foldEnrichment_first [1:10]
foldEnrichment_second [1:10]
foldEnrichment_firstANDsecondPeakSets [1:10]
foldEnrichment_secondANDfirstPeakSets [1:10]
```

```{R question 3.17}
order(firstANDsecondPeakSets$fold_enrichment, decreasing = TRUE)[1:500]
```

```{R question 3.17}
firstANDsecondPeakSets <- resize(firstANDsecondPeakSets,200,fix = "center")
firstANDsecondPeakSets[1:6]
```
...
```{R question 3.17}
commonPeaks_GR <- GRanges(seqnames = commonPeaks[,"chr"], IRanges(commonPeaks[,"start"], commonPeaks[,"end"]))
mcols(commonPeaks) <- commonPeaks[,c("abs_summit","fold_enrichment")]
```